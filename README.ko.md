...
**설치만으로는 안 됩니다 — 불러야 합니다.** Claude Code·`sonnet`에서 측정한 결과, 이 스킬을 위해 만든 픽스처에서 스스로 발동한 횟수는 5회 중 0회였고, description을 두 번 다시 써도 마찬가지였습니다. 같은 하네스에서 대조용 문구는 발동했으므로 자동 호출 자체가 죽은 것은 아닙니다 ([기록](evals/results/2026-08-28-trigger-experiment.md)). 스킬을 고르는 쪽은 **요청이 무엇에 관한 것인지**를 봅니다. 이 스킬의 트리거는 **당신이 곧 하려는 행동**이고, 그건 그 선택 과정에 보이지 않습니다.

그러니 필요한 순간에 소리 내어 말하세요:

```text
결정하기 전에 fresh-eyes-check 써줘.
```

compaction 직후, 되돌릴 수 없는 작업 직전, 또는 에이전트가 "예전에 그렇게 말씀하셔서"를 이유로 대는 걸 발견했을 때입니다. 이름을 대고 부르면 이월된 지시를 3/3으로 문제 삼았고 결정을 3/3으로 사용자에게 되돌렸습니다 (없는 쪽은 각각 0/3).
...
- [Live A/B](evals/results/2026-08-28-live-ab.md) — 같은 픽스처를 스킬 있는 쪽과 없는 쪽으로 각각 3회씩 돌리고, 어느 쪽이 어느 암인지 모르는 Codex 심판이 채점했습니다. 두 가지가 나왔습니다. **스킬은 스스로 뜨지 않습니다:** 설치되고 목록에 있고 호출 가능한 상태의 후보 6회 중 한 번도 호출되지 않았고, 답변은 스킬 없는 쪽과 구분되지 않았습니다. **이름을 대고 부르면 약속한 대로 동작합니다:** carried instruction 문제제기 3/3, 결정을 사용자에게 되돌림 3/3 (없는 쪽은 각각 0/3), 그리고 지시가 여전히 맞는 픽스처에서는 3/3 조용했습니다.

A/B는 여기서 유일하게 대조군이 있는 기록입니다. 그 앞의 네 건은 스킬을 **가진** 에이전트가 무엇을 했는지를 보여줄 뿐이며, 그것은 더 약한 증거입니다. 모든 조건이 한 대의 기계, 하나의 배우 모델, 작은 표본이고, 스킬·픽스처·루브릭을 쓴 사람이 같습니다. 기록은 관찰한 것을 적고 거기서 멈춥니다.

**이 저장소가 주장하지 않는 것:** 설치만 하면 에이전트의 행동이 달라진다는 것. 직접 측정한 결과 달라지지 않습니다 — 불러야 합니다. 이걸 고치는 것이 다음 작업이고, 손댈 곳은 본문이 아니라 description입니다.
